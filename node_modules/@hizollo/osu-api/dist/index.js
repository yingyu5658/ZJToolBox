...
__export(osu_api_exports, {
  ...
  MatchScoringType: () => MatchScoringType,
  MatchTeamType: () => MatchTeamType,
  Mods: () => Mods,
  ModsAbbreviation: () => ModsAbbreviation,
...
module.exports = __toCommonJS(osu_api_exports);

// src/Client.ts
var import_superagent2 = __toESM(require("superagent"));
...
var UserManager = class extends BaseManager {
  ...
  async fetchBanner({ id }) {
    const context = await import_superagent.default.get(`https://osu.ppy.sh/users/${id}`);
...
var Replay = class {
  constructor(client, data) {
    ...
    this.stream = import_node_buffer.Buffer.from(this.content, "base64");
  }
};
...
var Client = class {
  ...
  async request(options) {
    ...
    const response = await import_superagent2.default.get(`https://osu.ppy.sh/api/${options.path}`).query({ k: this.apiKey }).query(options.queries);
